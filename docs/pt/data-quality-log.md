...
## DQ-008: Campo "downloaded" nunca consultado por nenhuma etapa seguinte (01_download.py)

**Identificado em:** reflexão disparada por uma reflexao sobre se `04_translate.py` deveria validar o título antes de traduzir, igual ao check que fiz para descrição em `05_translate_descriptions.py`. A investigação dessa pergunta específica mostrou que o título não pode vir malformado (é filtrado na própria raspagem da listagem), mas a reflexão seguinte revelou o problema real: não era sobre o título isoladamente, era que nenhuma etapa depois de `01_download.py` checava o `catalog` como um todo antes de processar um registro (achado preventivo, não incidente observado: nos 10 e depois 20 livros reais processados, todos os downloads tiveram sucesso).

**Sintoma:** `01_download.py` grava `entry["downloaded"] = True/False` em cada registro de `catalog.json`, mas nenhuma etapa seguinte consulta esse campo antes de processar o registro. `04_translate.py`, `07_localized_catalog.py` e `08_universal_metadata.py` iteram sobre `catalog` inteiro sem checar `downloaded`.
