...
**Contexto:**
O desafio exige a escolha de pelo menos 2 das 5 target areas definidas (Event-Driven Pipeline, Data Architecture, Versioning, Scalability, Data Quality), com avaliação focada em decisões de engenharia, não nos scripts fornecidos. Minha escolha considerou o conhecimento técnico e também alinhamento da vaga de Data Foundry Engineer.

**Decisão:**
Escolhidas as target areas Data Quality e Event-Driven Pipeline. Data Quality foi selecionada por conhecimento e experiência prévia, e também porque a leitura do case deixou claro que era uma das target areas mais adequadas a esse cenário específico, dados raspados de uma fonte pública e sujeitos a problemas reais de qualidade. Event-Driven Pipeline foi selecionada por ser uma oportunidade concreta de aprendizado dentro do escopo do teste, e também porque a leitura do `main.py` original revelou que a execução do pipeline, em ordem fixa, poderia ser melhor gerenciada e otimizada reagindo a eventos entre as etapas, em vez de depender de uma ordem estática. Data Architecture foi avaliada e descartada como target area formal: o projeto tem uma única fonte de dados e apenas dois arquivos finais exigidos pelo case, sem múltiplas fontes para integrar nem consumo por BI/analytics/ML que justificasse formalizar camadas adicionais de agregação (por exemplo, um modelo bronze/silver/gold ou data marts).
