...
**Ideia:** Usar o mesmo campo de status/sinalização (Data Quality) como condição de disparo para o Event-Driven Pipeline — por exemplo, só processar a tradução de descrição de um livro se a etapa de descrição daquele livro tiver sido concluída com sucesso. Isso conecta diretamente os dois pilares escolhidos: o sinal de qualidade de um estágio vira o gatilho de execução do próximo.
